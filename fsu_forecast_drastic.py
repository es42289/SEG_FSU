...
all_owners["alias"] = all_owners.apply(lambda row: make_alias(row["Owner"], row["WELL_COUNT"]), axis=1)
## selection box for owner filtering
owner_filter = st.text_input("Enter owner name (or part of it)", "FAITH RESOURCES")
## if an owner is selected then....
if owner_filter:
    ## limit owners to what is in search filter
    ...
    ## if owner list is less than 200 (manageable fo rdropdown)
    if owners.shape[0]<=200:
        ## selection box for owner
        selected_alias = st.selectbox("Filter by Owner", options=list(alias_to_owner.keys()))
        ## convert the alias back to the owner name
        owner = alias_to_owner[selected_alias]
                ...
                with st.expander('Oil Decline Parameters', expanded=False):
                    ...
                    with mid_left_col:
                        ...
                        params['OIL_USER_QI'] = st.slider(
                            ...
                            max_value=1.25*max(prd[prd['PRODUCINGMONTH']>datetime(params['FCST_START_OIL'].year, params['FCST_START_OIL'].month, params['FCST_START_OIL'].day)]['LIQUIDSPROD_BBL']),
        ...
        st.warning("Please enter more chacters to narrow the owner search")
        st.write("Below is a full list of Owners")
